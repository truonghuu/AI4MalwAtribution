...
if __name__ == '__main__':
    ...
    with (zipfile.ZipFile(PATH, 'r')) as zipper:
        ...
        for file_info in zipper.infolist():
            ...
            if(not os.path.exists('newNpy/'+output_file_name)):
                zipper.extract(file_info, 'unzipped')
                print(f"about to dmds: {file_info.filename}")

                dmds = DMDS(input_file_name, input_path='unzipped/' + input_file_name, output_path='newNpy/'+output_file_name,max_len=1000,idx = 40)
                print(dmds.file_name)
                print(dmds.input_path)
                print(dmds.output_path)
